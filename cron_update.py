...
def update_data(day_range = 0):
    ...
    end_date_epochmille = int(((now.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)).timestamp() + 3600*3) * 1000)
    result_data = {}
        ...
        for unit in division['units']:
            ...
            for endpoint, key, response_key, from_param_name, to_param_name in endpoints:
                ...
                if response.get(response_key):
                    result[key] = response[response_key]

            params = {"from": (now-timedelta(days=7)).strftime("%Y-%m-%d"), "to": (now-timedelta(days=7)).strftime("%Y-%m-%dT%H:%M:%S"), "units": unit_id}
            response = DodoIS._request(endpoint="finances/sales/units", params=params)
            if response.get("result"):
                result["salesStatisticsWeekAgo"] = response["result"]
            ...
            if trendyol_unit_id:
                ...
                for package in response:
                    ...
                    if package['packageStatus'] == "Cancelled" or package['packageStatus'] == "UnSupplied" and not package['cancelInfo']:
                        trendyol_order_data['cancelled_orders'].append({"reason" : package['cancelInfo'], "orderId":package['orderId'], "totalPrice" : package['totalPrice']})

                    if package['packageStatus'] == "Delivered" and package['packageCreationDate'] < package['packageModificationDate'] - 3600 * 1000:
                        trendyol_order_data['late_orders'].append(
                            {"orderId":package['orderId'],
                           "late_time" : int((package['packageModificationDate']-package['packageCreationDate'])/60/1000)})
                    trendyol_order_data['order_price_coordinate'].append([package['totalPrice'],package['address']['latitude'],package['address']['longitude']])

                if trendyol_order_data['total_order']:
                    trendyol_result['orders'] = trendyol_order_data

                if trendyol_result:
                    result['trendyol'] = trendyol_result
            ...
            if yemeksepeti_unit_id:
                ...
                if total_order:
                    ...
                    for order in order_list:
                        ...
                        created_at_local = created_at_utc.astimezone(gmt_plus_3)
                        ...
                        if order_detail['status'] == "cancelled":
                            yemeksepeti_order_data['cancelled_orders'].append({"orderId":order_detail['code'],'price':order_detail['price']['totalNet']})
                            continue
                        price = float(order_detail['price']['totalNet'])
                        yemeksepeti_order_data['total_price'] += price


                        yemeksepeti_order_data['order_price_coordinate'].append([price,order_detail['delivery']['address']['latitude'],order_detail['delivery']['address']['longitude']])
                    yemeksepeti_result['orders'] = yemeksepeti_order_data

                if yemeksepeti_result:
                    result['yemeksepeti'] = yemeksepeti_result
